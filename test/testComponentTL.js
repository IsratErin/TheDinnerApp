...
import {recordArrayCB,stopRecording} from "./checkCB.js";
...
function prepareTest(components, framework, props, test){
    ...
    let output;
    try{
        output= renderers[framework].renderWithProps(components[framework], props, myDiv);
    }finally{stopRecording();}
    if(!myDiv.children.length && myDiv.innerText.length)
        // this is probably a dummy view, has no children and returns some text
        test.skip();
