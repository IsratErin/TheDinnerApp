...
const pyramid= (await import(`../lab-pyramid.txt?raw`)).default;
console.log(pyramid);

const oldConsoleWarn= console.warn;
const oldConsoleError= console.error;
const WrongRoute= "/pursposefully_wrong_route";
function testWarnCB(param){
    return param.indexOf &&(
	param.indexOf(WrongRoute)!=-1 ||
	    param.indexOf("React Router Future Flag Warning")!=-1);
}
...
function testErrorCB(x){
    return testWarnCB(x) || x.data && testWarnCB(x.data)
}

/*console.error= function(...params){
    if(!params.find(testErrorCB))
	oldConsoleError(...params);
}*/
...
class TestAnalyzer {
  ...
  formatAndPostTestResults() {
    ...
    // to force telemetry during testing, put a breakpoint here and assign TELEMETRY at the Console
    if (TELEMETRY !== "none") this.postData(body);
    else console.log("skipping telemetry");  
  }
