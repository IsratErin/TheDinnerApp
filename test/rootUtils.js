...
function makeCreateElement(framework, h) {
  ...
  return {
      ...
      if (tag.name == "SuspenseView") {
        return h(Dummy, { ...props, type: "suspense" });
      }
      if (
        tag == "div" &&
        children &&
...
async function testVue(theTest) {
  ...
  if (!VueRootAll?.makeRouter && theTest===testRoutes)
     return false;
...
async function testReact(theTest) {
  ...
  if(!foundReactRouter && theTest===testRoutes)
       return false;  
  try {
    const result = await theTest(propsHistory, model);
    if (result === false) return false;
...
async function testSuspense(propsHistory, model) {
  ...
  expect(
    propsHistory[propsHistory.length - 1].type,
      "the root component will render SuspenseView while the model is not ready"
  ).to.equal("suspense");
    expect(
	propsHistory[propsHistory.length - 1].promise,
	"SuspenseView renered by the root has a dummy promise, so that an image is rendered" 
    ).to.be.ok;
...
async function testWeek1Presenters(propsHistory, model) {
    ...
    expect(
        propsHistory[0]?.type,
        "Sidebar is supposed to be rendered first, before Summary and other presenters (if any)"
    ).to.equal("sidebar");
    return true;
}
